...
| D-023 | 2026-08-25 | Repository scaffolding: git init, top-level README (essence, Five Laws, layout, quick start), .gitignore; ADR numbering collision fixed (memory + single-hypothesis merged into ADR-002). Verified post-restructure: 21/21 tests, deterministic cycle passes | Accepted |
| D-024 | 2026-08-25 | First LIVE cycle: real LLM (OpenRouter free tier) drove the control plane. LLM self-corrected through 3 INVALID specs to VALID→ISSUED — Laws held, no violations. Blocked by free-tier daily limit (50 req/day) before result ingestion; multi-turn host-runner loop implemented for completion. Fixes: SQLite check_same_thread=False, retries on upstream 502/429 | Accepted |
