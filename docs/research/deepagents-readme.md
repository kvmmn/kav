...
# KAV Notes on Deep Agents (2026-08-25)

## Why this matters for KAV
Deep Agents is LangChain's opinionated "agent harness" built on LangGraph. KAV's Control Plane
could either (a) build directly on raw LangGraph, or (b) build on deepagents and customize.

## Relevant deepagents features mapped to KAV needs
| deepagents feature | KAV mapping |
|---|---|
| Sub-agents (isolated context) | Hypothesis generator / experiment designer / result analyst as sub-agents |
| Filesystem backend | Experiment artifacts, specs, evidence storage |
| Context management | Long research histories without context blowup |
| Persistent memory | ResearchFinding / Knowledge Memory |
| Human-in-the-loop tool approval | Principle 8.10: human control at high-risk boundaries (promotion, deployment) |
| Skills | Reusable research strategies / priors |
| Model-agnostic | No vendor lock-in for the reasoning layer |

## Open question for ADR
Build KAV Control Plane on **raw LangGraph** (master doc's current stance) vs **deepagents harness**
(faster start, but inherits opinions we may not want — e.g., filesystem-centric tools, coding bias).
→ Candidate first ADR.
